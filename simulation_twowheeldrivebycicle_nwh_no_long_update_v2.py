...
def oneStep(X0):    
    ...
    opti.subject_to(opti.bounded(0,U[1],1))   
...
def optimize(X0,forward_N):  
    ...
    X_dot[6,:] = delta_dot_sym_array
       ...
       #objective    
    opti.minimize(-50*(tau_sym_array[-1] -tau0)
        + casadi.dot(delta_dot_sym_array,delta_dot_sym_array)*0.0001/N 
        + n_sym_array[-1]*n_sym_array[-1]*0.0001/N)
        #+ casadi.dot((fx_f_sym_array[1:]-fx_f_sym_array[0:-1]),(fx_f_sym_array[1:]-fx_f_sym_array[0:-1]))*0.0000001/N
        #+ casadi.dot((fx_r_sym_array[1:]-fx_r_sym_array[0:-1]),(fx_r_sym_array[1:]-fx_r_sym_array[0:-1]))*0.0000001/N)
    ...
    opti.subject_to(opti.bounded(-track_width/2,n_sym_array[1:-1],track_width/2))
