...
cur.execute("CREATE TABLE {} (computation_time real,phi real, vx real, vy real, steer real,omega real,front_wheel_alpha real,rear_wheel_alpha real,front_fx real, rear_fx real,front_fy real, rear_fy real,s real,ds real,poly_order real,ptx text,pty text,coeff text)".format(table_name))
...
def optimize(y0):  
    ...
    tau0,_ = track.convertXYtoTN([float(y0[0]),float(y0[1])])
    ...
    try:
        s0_new,n0_new = convertXYtoSN(track,coeff,y0[0:2])
    except Exception as e:
        print(e)
    X0 = casadi.veccat(float(s0_new),float(n0_new),float(y0[2]),float(y0[3]),float(y0[4]),float(y0[5]),float(y0[6]))
    ...
    cur.execute(sql_query,
        (computation_time,sql_phi,sql_vx,sql_vy,sql_steer,sql_omega,sql_front_wheel_alpha,sql_rear_wheel_alpha,sql_front_fx,sql_rear_fx,sql_front_fy,sql_rear_fy,s0,ds,float(best_order),sql_ptx,sql_pty,sql_coeff))
    con.commit()
