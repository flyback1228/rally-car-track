...
def optimize(X0):  
    ...
    try:
        ...
        #input bound
        opti.subject_to(opti.bounded(delta_dot_min,delta_dot_sym_array,delta_dot_max))
        opti.subject_to(opti.bounded(-3,front_throttle_array,1) ) 
        opti.subject_to(opti.bounded(-3,rear_throttle_array,1) ) 
        ...
        opti.solver("ipopt",{},except_option) # set numerical backend
        try:
            sol = opti.solve()   
        except Exception as e:
            exit()
