...
import yaml
from track import *
from dynamics_models import BicycleDynamicsModelTwoWheelDriveWithBrakeNWHNoLong
...
model = BicycleDynamicsModelTwoWheelDriveWithBrakeNWHNoLong(params,track,front_tire_model,rear_tire_model)
print(params)
...
cur.execute("CREATE TABLE {} (computation_time real,phi real, vx real, vy real, steer real,d real,omega real,front_wheel_alpha real,rear_wheel_alpha real,front_brake real, rear_brake real,ptx text,pty text,error text)".format(table_name))
#ocp params0.01
N = 60
nx = model.nx
nu = model.nu
...
def optimize(X0,forward_N):  
    ...
    opti.minimize(-0.1*(tau[-1]-ref_tau[-1]) + casadi.dot(delta_dot,delta_dot)*0.0001 + casadi.dot(front_brake,front_brake)*0.0001 +casadi.dot(rear_brake,rear_brake)*0.0001)# +casadi.dot(d,d)*0.00001 )
    #initial condition
    ...
    #state bound
    opti.subject_to(opti.bounded(0,vx,v_max))
    ...
    opti.subject_to(opti.bounded(0,d,1))
    ...
    try:
        ...
        cur.execute(sql_query,
            (computation_time,sql_phi,sql_vx,sql_vy,sql_steer,sql_d,sql_omega,sql_front_wheel_alpha,sql_rear_wheel_alpha,sql_front_brake,sql_rear_brake,sql_ptx,sql_pty,error))
        con.commit()
