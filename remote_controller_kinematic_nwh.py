        ...
        async for message in ws:
            ...
            if _check_mode(message) == 'Auto':
                ...
                if (not msg_type):
                    ...
                    opti.minimize(-4 * (tau[-1] - ref_tau[-1]) + 0.00002 * casadi.dot(n, n) + 10.0 * casadi.dot(
                        delta[1:] - delta[0:-1], delta[1:] - delta[0:-1]))
                    ...
                    cur.execute(sql_query,
                                (float(telemetry['sending_time']) - start_time, receive_time, send_time,
                                 float(telemetry['applying_time']) - start_time, x, y, phi0, v0, steering, throttle,
                                 steering, throttle))
                    con.commit()
                    await ws.send(msg)
