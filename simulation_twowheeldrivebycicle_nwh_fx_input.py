...
def oneStep(X0):    
    ...
    opti.subject_to(opti.bounded(-Fx_f_max,U[1],Fx_f_max))   
    opti.subject_to(opti.bounded(-Fx_r_max,U[2],Fx_r_max))    
...
def optimize(X0,forward_N):  
    ...
    opti.minimize(-0.1*(tau[-1]-ref_tau[-1]) + casadi.dot(delta_dot,delta_dot)*0.0001)# +casadi.dot(d,d)*0.00001 )
    #initial condition
    ...
    opti.subject_to(opti.bounded(-Fx_f_max,fx_f,Fx_f_max))   
    opti.subject_to(opti.bounded(-Fx_r_max,fx_r,Fx_r_max))   
    ...
    try:
        ...
        sol_front_fx = sol.value(fx_f)
        sol_rear_fx = sol.value(fx_r)
        ...
        if(e.args[0].find('Infeasible_Problem_Detected')!=-1):
            ...
            sol_front_fx = opti.debug.value(fx_f)
            sol_rear_fx = opti.debug.value(fx_r)
