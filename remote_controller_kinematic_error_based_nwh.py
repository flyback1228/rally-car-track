...
import yaml
from dynamics_models import BicycleKineticModelByParametricArcWithBrake
import time
import casadi
import numpy as np
import sqlite3
...
model = BicycleKineticModelByParametricArcWithBrake(params,track)
...
def getRef(x0):
    ...
    for i in range(N):
        for j in range(10):
            ...
            st = (s0 + v0*estimate_dt/10)%track.max_s
            tau_t = track.getTFromS(st)
            kappt_t = casadi.fabs(f(tau_t))
            v0=(2000.0*kappt_t*kappt_t-80*kappt_t+1)*v_max
            s0 = st
...
cur.execute("CREATE TABLE {} (unity_sent_time real, python_receved_time real, python_sent_time real, unity_received_type real,real_x real,real_y real,real_psi real,real_v real,received_steer real, received_throttle real, sent_steer real, sent_throttle real,sent_front_brake real,sent_rear_brake real)".format(table_name))
...
async def control_loop(ws):
    global steering,throttle,last_mpc_u,last_mpc_x,x_guess,u_guess,option,estimate_dt,start_time,first_it,send_time,brake
    async for message in ws:
        ...
        if _check_mode(message) == 'Auto':                
            ...
            if(msg_type):
                ...
                real_u = casadi.DM([steering,throttle,brake])
                ...
                opti.subject_to(opti.bounded(0,d,d_max))  
                opti.subject_to(opti.bounded(0,b,1))                   
                ...
                try:      
                    ...
                    data['brake'] = f"{brake:0.2f},{brake:0.2f}" 
                ...
                cur.execute(sql_query,
                    (float(telemetry['sending_time'])-start_time,receive_time,send_time,applying_time-start_time,x,y,phi0,v0,received_steering,received_throttle,steering,sql_throttle,front_brake,rear_brake))
                con.commit()
